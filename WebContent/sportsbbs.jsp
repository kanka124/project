	...
	<div class="container">
		<div class="row">
			<table class="table table-striped"
				...
				<tbody>
					<%
						sportsbbsDao bbsDAO = new sportsbbsDao();
						
						ArrayList<sportsbbs> list = bbsDAO.getList(pageNumber);
						for (int i = 0; i < list.size(); i++) {
					%>
					<tr>
						<td><%=list.get(i).getBbsID()%></td>
						<td><a href="sportsbbsView.jsp?bbsID=<%=list.get(i).getBbsID()%>">
			...
			<%
				if (pageNumber != 1) {
			%>
			<a href="sportsbbs.jsp?pageNumber=<%=pageNumber - 1%>"
			...
			<%
				...
				if (session.getAttribute("userID") != null) {
			%>
			<a href="sportsbbsWrite.jsp" class="btn btn-primary pull-right">글쓰기</a>
			<%
				} else {
			%>
