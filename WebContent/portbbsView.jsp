...
<%@ page import="blogs.bbs.portbbs"%>
<%@ page import="blogs.bbs.portbbsDao"%>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
...
<%
	//로긴 한사람이면 userID라는 변수에 해당 아이디가 담기고 그렇지 않으면 null값
		...
		portbbs bbs = new portbbsDao().getBbs(bbsID);
	%>
...
<!-- .replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;")
			...
			<a href="eatbbs.jsp" class="btn btn-primary">목록</a>
			<%
				//글작성자 본인일시 수정 삭제 가능
				if (userID != null && userID.equals(bbs.getUserID())) {
			%>
			<a href="portbbsUpdate.jsp?bbsID=<%=bbsID%>" class="btn btn-primary">수정</a>
			<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="portbbsdeleteAction.jsp?bbsID=<%= bbsID %>" class="btn btn-primary">삭제</a>
			<%
				}
			%>
		</div>
