...
<%@ page import="blogs.bbs.eatbbs"%>
<%@ page import="blogs.bbs.eatbbsDao"%>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
...
<%
	//로긴 한사람이면 userID라는 변수에 해당 아이디가 담기고 그렇지 않으면 null값
		...
		eatbbs bbs = new eatbbsDao().getBbs(bbsID);
	%>
...
<!-- .replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;")
			...
			%>
			<a href="eatbbsUpdate.jsp?bbsID=<%=bbsID%>" class="btn btn-primary">수정</a>
			<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="eatbbsdeleteAction.jsp?bbsID=<%= bbsID %>" class="btn btn-primary">삭제</a>
			<%
				}
			%>
		</div>
