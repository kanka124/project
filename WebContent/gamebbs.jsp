	...
	<div class="container">
		<div class="row">
			<table class="table table-striped"
				...
				<tbody>
					<%
						gamebbsDao bbsDAO = new gamebbsDao();
						
						ArrayList<gamebbs> list = bbsDAO.getList(pageNumber);
						for (int i = 0; i < list.size(); i++) {
					%>
					<tr>
						<td><%=list.get(i).getBbsID()%></td>
						<td><a href="gamebbsView.jsp?bbsID=<%=list.get(i).getBbsID()%>">
			...
			<%
				if (pageNumber != 1) {
			%>
			<a href="gamebbs.jsp?pageNumber=<%=pageNumber - 1%>"
			...
			<%
				...
				if (session.getAttribute("userID") != null) {
			%>
			<a href="gamebbsWrite.jsp" class="btn btn-primary pull-right">글쓰기</a>
			<%
				} else {
			%>
