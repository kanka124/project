	...
	<div class="container">
		<div class="row">
			<table class="table table-striped"
				...
				<tbody>
					<%
						eatbbsDao bbsDAO = new eatbbsDao();
						
						ArrayList<eatbbs> list = bbsDAO.getList(pageNumber);
						for (int i = 0; i < list.size(); i++) {
					%>
					<tr>
						<td><%=list.get(i).getBbsID()%></td>
						<td><a href="eatbbsView.jsp?bbsID=<%=list.get(i).getBbsID()%>">
			...
			<%
				if (pageNumber != 1) {
			%>
			<a href="eatbbs.jsp?pageNumber=<%=pageNumber - 1%>"
			...
			<%
				...
				if (session.getAttribute("userID") != null) {
			%>
			<a href="eatbbsWrite.jsp" class="btn btn-primary pull-right">글쓰기</a>
			<%
				} else {
			%>
