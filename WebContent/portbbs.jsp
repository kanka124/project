	...
	<div class="container">
		<div class="row">
			<table class="table table-striped"
				...
				<tbody>
					<%
						portbbsDao bbsDAO = new portbbsDao();
						
						ArrayList<portbbs> list = bbsDAO.getList(pageNumber);
						for (int i = 0; i < list.size(); i++) {
					%>
					<tr>
						<td><%=list.get(i).getBbsID()%></td>
						<td><a href="portbbsView.jsp?bbsID=<%=list.get(i).getBbsID()%>">
			...
			<%
				if (pageNumber != 1) {
			%>
			<a href="portbbs.jsp?pageNumber=<%=pageNumber - 1%>"
			...
			<%
				...
				if (session.getAttribute("userID") != null) {
			%>
			<a href="portbbsWrite.jsp" class="btn btn-primary pull-right">글쓰기</a>
			<%
				} else {
			%>
